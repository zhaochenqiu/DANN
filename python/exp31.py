...
class ClassNet(nn.Module):
    ...
    def __init__(self, input_size=1000, hidden_size=20000):
        super().__init__()
...
def main():
    ...
    with torch.no_grad():
        ...
        X1 = torch.empty(num).normal_(mean = 1, std = 2)
        X2 = torch.empty(num).normal_(mean = -1, std = 2)
    ...
    for i in range(1000):
        ...
        if flag == 0:
            ...
            c_Z2, f_Z2 = prodis_lay(c_X1, f_X1, c_W, f_W, c_Z, f_Z, border, params)

            output = classnet(f_Z2)

            loss = loss_func(output, target2)
