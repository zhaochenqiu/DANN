...
#     network.load_state_dict(torch.load(name_net))
    ...
    for frame_idx in curidx:
        ...
        starttime = time.time()
        re_labs = test(data_vid, 2000, device, prodis_mul, c_data, network,
            c_W, f_W_R, f_W_G, f_W_B,
            c_B, f_B_R, f_B_G, f_B_B, c_Z, delta, params)

        endtime = time.time()

        print("total 2:", endtime - starttime)
...
# frames, row_im, column_im, byte_im
    ...
    #            c_W, f_W_R, f_W_G, f_W_B, c_Z, delta, params)
            ...
            starttime = time.time()
            re_labs = test(data_vid, batchsize*4, device, prodis_mul, c_data, network,
                c_W, f_W_R, f_W_G, f_W_B,
                c_B, f_B_R, f_B_G, f_B_B, c_Z, delta, params)

            endtime = time.time()

            time_detect = endtime - starttime
