...
#        x = F.relu(self.fc3(self.fc1(input)))
...
def main():
    ...
    with torch.no_grad():
        ...
        X2 = torch.empty(num).normal_(mean = 0, std = 1.1)


        c_X1, f_X1 = getHist_plus(X1, border, -50.0, 50.0)
        c_X2, f_X2 = getHist_plus(X2, border, -50.0, 50.0)



        corval = corDisVal(c_X1, f_X1, c_X2, f_X2)
    ...
    c_W, f_W = getRandCF(left, right, border)
...
#    optim_W = optim.SGD([f_W], lr=0.01, momentum=0.9)
    ...
    optim_net = optim.Adam(classnet.parameters(), lr = 0.01)
#    optim_net = optim.SGD(classnet.parameters(), lr=0.001, momentum=0.5)


    class_weights = torch.FloatTensor([0.6, 0.4]).to(device)
...
#     print("f_X1.shape = ", f_X1.shape)
#     print("f_X2.shape = ", f_X2.shape)
    ...
    print("f_Z1 = ", torch.sum(f_Z1))
    print("f_Z2 = ", torch.sum(f_Z2))
