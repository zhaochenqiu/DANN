...
#           c_data, c_W, f_W, c_B, f_B, c_W_S, f_W_S, c_B_S, f_B_S, c_Z, delta, params,
#           optim_W, optim_B, optim_W_S, optim_B_S,
...
#     print("borderline ------------------------------")
    ...
    if cnt > -1:
        ...
        name_f_W_S = net_pa + "f_W_S_" + str(cnt).zfill(4) + ".pt"
        name_f_B_S = net_pa + "f_B_S_" + str(cnt).zfill(4) + ".pt"
        ...
        f_W_S = torch.load(name_f_W_S)
        f_B_S = torch.load(name_f_B_S)
    ...
    for epoch in range(cnt + 1, num_epoch):
        ...
        for i in range(value):
            ...
            f_Z_W = torch.cat( [prodis_mul(c_data, data[:, :, b], c_W, f_W[:, :, b], c_Z, delta, params)[1].unsqueeze(-1)
                                for b in range(f_W.shape[-1])], dim = 3 )

            f_Z_B = torch.cat( [difdis_mul(c_data, data[:, :, b], c_B, f_B[:, :, b], c_Z, delta, params)[1].unsqueeze(-1)
                                for b in range(f_B.shape[-1])], dim = 3 )
            ...
            f_Z_W_S = torch.cat( [torch.cat( [prodis_mul(c_Z, f_F[:, t, :, b].squeeze(), c_W_S, f_W_S[:, :, b], c_Z, delta, params)[1].unsqueeze(-1)
                                  for b in range(f_W_S.shape[-1])], dim = 3 ) for t in range(2)], dim = 1)

            f_Z_B_S = torch.cat( [torch.cat( [difdis_mul(c_Z, f_F[:, t, :, b].squeeze(), c_B_S, f_B_S[:, :, b], c_Z, delta, params)[1].unsqueeze(-1)
                                  for b in range(f_B_S.shape[-1])], dim = 3 ) for t in range(2)], dim = 1)



            f_F_S = f_Z_W_S + f_Z_B_S


            f_F_S = f_F_S.permute(0, 3, 1, 2)

            output = network(f_F_S)
        ...
        if epoch % 1 == 0:
            ...
            name_f_W_S = net_pa + "f_W_S_" + str(epoch).zfill(4) + ".pt"
            name_f_B_S = net_pa + "f_B_S_" + str(epoch).zfill(4) + ".pt"


            saveMod(name_f_W, f_W)
            saveMod(name_f_B, f_B)

            saveMod(name_f_W_S, f_W_S)
            saveMod(name_f_B_S, f_B_S)
    ...
    return network, f_W, f_B, f_W_S, f_B_S



def test(data_vid, batchsize, device, prodis_mul, difdis_mul, c_data, network,
            c_W, f_W,
         c_B, f_B, c_W_S, f_W_S, c_B_S, f_B_S, c_Z, delta, params):
...
class ClassifyNetwork(nn.Module):
    def __init__(self, dis_num):
        ...
        self.conv1 = nn.Conv2d(3, 20, (dis_num*3, 1) )
        self.conv2 = nn.Conv2d(20, 1000, (1, 201))
...
#     net_pa = '../../data/'
#     sa_pa = '../../data/results/'
    net_pa = '../../data/network_fountain01_v31/'
    sa_pa =  '../../data/fgimgs_fountain01_v31/'
...
#    print(torch.sum(torch.abs(vid_hist - vid_hist_pad)))
    ...
    num_dis_S = 3
    c_W_S, f_W_S = getRandCF_multi(left, right, delta, num_dis_S, 3)
    c_B_S, f_B_S = getRandCF_multi(left, right, delta, num_dis_S, 3)
    ...
    c_W_S = c_W_S.to(device)
    c_B_S = c_B_S.to(device)

    f_W_S = f_W_S.to(device)
    f_B_S = f_B_S.to(device)
    ...
    f_W_S = Variable(f_W_S, requires_grad = True)
    optim_W_S = optim.Adam([f_W_S], lr = 0.001, amsgrad = True)

    f_B_S = Variable(f_B_S, requires_grad = True)
    optim_B_S = optim.Adam([f_B_S], lr = 0.001, amsgrad = True)
    ...
    network, f_W, f_B, f_W_S, f_B_S = train(data_vid, labs_vid, batchsize, device, network, loss_func, prodis_mul, difdis_mul,
          c_data, c_W, f_W, c_B, f_B, c_W_S, f_W_S, c_B_S, f_B_S, c_Z, delta, params,
          optim_W, optim_B, optim_W_S, optim_B_S,
          optim_net, net_pa, num_epoch)
...
# test(data_vid, batchsize, device, prodis_mul, difdis_mul, c_data, network,
#             c_W, f_W,
#          c_B, f_B, c_W_S, f_W_S, c_B_S, f_B_S, c_Z, delta, params):
#
