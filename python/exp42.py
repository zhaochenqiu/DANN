...
#    prodis_lay = ProductDis.apply
    prodis_lay = ProductDis_fast.apply
    ...
    c_Z, f_Z = getEmptyCF(left, right, delta)
    ...
    classnet = ClassNet(402,2000).to(device)
    ...
    for i in range(500):
        ...
        if flag == 0:
            ...
            c_Z1, f_Z1 = prodis_lay(c_X2, f_X2, c_W, f_W[0], c_Z, f_Z, border, params)
            c_Z2, f_Z2 = prodis_lay(c_X2, f_X2, c_W, f_W[1], c_Z, f_Z, border, params)
    ...
    plt.bar(c_W.detach().cpu().numpy(),
            f_W[0].detach().cpu().numpy(),
            width = 0.1,
            color = (0.1, 0.1, 0.1, 0.5))

    plt.subplot(1, 2, 2)
    plt.bar(c_W.detach().cpu().numpy(),
            f_W[1].detach().cpu().numpy(),
            width = 0.1,
            color = (0.1, 0.1, 0.1, 0.5))
