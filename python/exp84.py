...
#    curidx = [1140]
    ...
    data_vid, labs_vid = getListNormalData_plus(imgs, labs, curidx, "test", left_data, right_data, delta)
    print("completed")

    labs_vid = torch.round(labs_vid/255)

    im = imgs[1148, :, :, :].reshape(row_im*column_im, byte_im)
...
#    starttime = datetime.datetime.now()
    ...
    for i in range(100):
        ...
        idx = (im_r > (i - 2)) & (im_r < i + 1)

        # print("i = ", i)

        re_data[idx,i:i+99] = data[idx,1:100]

#    endtime = datetime.datetime.now()
    endtime = time.time()

#    print("total time:", (endtime - starttime).microsecond)
    print("total time:", endtime - starttime)

    starttime = time.time()
    data_vid, labs_vid = getNormalData_plus(imgs, labs, 1148, left, right, delta)
    endtime = time.time()
