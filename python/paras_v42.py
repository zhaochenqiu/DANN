...
from bayesian.bayesian import bayesRefine
...
#     net_pa = '../../data/'
#     sa_pa = '../../data/results/'
    net_pa = '../../data/network_fountain01_v34/'
    sa_pa =  '../../data/fgimgs_fountain01_v34/'
...
#     curidx = [640, 710, 720, 740, 1130, 1149, 1184]
#
#
...
#
    ...
    fs_fg, fullfs_fg = loadFiles_plus(sa_pa, ft_gt)
    ...
    for i in range(frames):
        im   = torch.tensor( imageio.imread(fullfs_im[i]), dtype = torch.float )
        fgim = torch.tensor( imageio.imread(fullfs_fg[i]), dtype = torch.float )
        gtim = torch.tensor( imageio.imread(fullfs_gt[i]), dtype = torch.float )

        starttime = time.time()
        bayfgim1 = bayesRefine(im, fgim,     radius, rate)
        endtime = time.time()
        print("time1: ", endtime - starttime)

        starttime = time.time()
        bayfgim2 = bayesRefine(im, bayfgim1, radius, rate)
        endtime = time.time()
        print("time2: ", endtime - starttime)


        starttime = time.time()
        bayfgim3 = bayesRefine(im, bayfgim2, radius, rate)
        endtime = time.time()
        print("time3: ", endtime - starttime)


        im_fg = np.round(fgim.detach().cpu().numpy())
        gt_fg = np.round(gtim.detach().cpu().numpy())
        ...
        print("current     Re, Pr, Fm:", Re, Pr, Fm)
        print("accumulated Re, Pr, Fm:", Re_sum, Pr_sum, Fm_sum)



        im_fg = np.round(bayfgim3.detach().cpu().numpy())
        gt_fg = np.round(gtim.detach().cpu().numpy())
        ...
        print("bay current     Re, Pr, Fm:", Re, Pr, Fm)
        print("bay accumulated Re, Pr, Fm:", Re_sum, Pr_sum, Fm_sum)
