...
class EZNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(100, 1024, 3)
...
for i in range(frames):
    imgblock[i, :, :, :] = imgblock[i, :, :, :] - curimg



imgblock = imgblock.transpose(1, 2, 0 ,3)

imgblock = np.reshape(imgblock, (row*column, frames, byte)  ).dot(1.0/255.0)
imglabel = np.reshape(labimg,   (row*column, 1)).squeeze().dot(1.0/255.0)


print('imgblock.shape = ', imgblock.shape)
print('imglabel.shape = ', imglabel.shape)


frame_data, num_data, byte_data = imgblock.shape


batch_size = 1000
epoch = 100

offset = frame_data%batch_size

length = frame_data - offset


data = torch.tensor(imgblock, dtype=torch.float)
labs = torch.tensor(imglabel, dtype=torch.int64)
labs = labs.squeeze()
...
for i in range(epoch):
    idx_all = np.asarray(range(frame_data))
    np.random.shuffle(idx_all)

    idx_len = idx_all[0:length]
    idx_ofs = idx_all[length:frame_data]


    idx_len = np.reshape(idx_len, (int(length/batch_size), batch_size))
    for cnt, idx in enumerate(idx_len):
        subdata = data[idx, :]
        sublabs = labs[idx]

        subdata = subdata.to(device, dtype=torch.float)
        sublabs = sublabs.to(device, dtype=torch.int64)

        optimizer.zero_grad()

        pre_labs = model(subdata)
        loss = loss_func(pre_labs, sublabs)

        loss.backward()
        optimizer.step()

        print("epoch: ", i, "  loss.item() = ", loss.item())



    subdata = data[idx_ofs, :]
    sublabs = labs[idx_ofs]
...
#     np.random.shuffle(idx_all)
#
#     idx_all = np.reshape(idx_all, (int(length/batch_size), batch_size))
#
#     for cnt, idx in enumerate(idx_all):
#        print("idx = ", idx)
# #        print("data = ", data)
#
#     idx = np.asarray(range(length, frame_data))
#     np.random.shuffle(idx)
#     print("idx = ", idx)

#
#     idx_all = np.asarray(range(length))
#     print(idx_all)
#     idx_all = np.asarray(range(length, frame_data))
#
#     print(idx_all)
...
#
#
# data = imgblock[50:100, :, :].dot(1.0/255.0)
# labs = imglabel[50:100].dot(1.0/255.0)
# labs.astype(int)
#
#
#
...
# # idx = np.asarray(idx)
# # np.random.shuffle(idx)
# # idx = np.reshape(idx, (10, 10))
# #
# # print(idx)
# #
# # for showidx, data in enumerate(idx):
# #     print("showidx = ", showidx)
# #     print("data = ", data)
# #
#
#
