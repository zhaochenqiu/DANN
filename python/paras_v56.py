...
from common_py.dataIO import loadImgs_pytorch_src
...
def getConvImgs(imgs):
    ...
    for i in range(frames_im):
        re_imgs[i] = gaussian(imgs[i, :, :, :].float().unsqueeze(dim = 0).permute(0, 3, 1, 2)).squeeze().permute(1, 2, 0).type(torch.uint8)
...
#    imgs = (imgs/255.0)*right
    ...
    for i in range(row*column):
        for b in range(byte):
            hist_data[i, :, b] = torch.histc((imgs[i, :, b].float()/255.0)*right, num_hist, left, right)/(frame*1.0)
...
def getNormalData_byHistVid(vid_hist, imgs, labs, curidx, left = -1, right = 1, delta = 0.01):
    ...
    im = imgs[curidx].reshape(row_im*column_im, byte_im).float()
    lb = labs[curidx].float()
...
#    print("cuda:" + str(gpuid))
    ...
    imgs = loadImgs_pytorch_src(pa_im, ft_im)
    labs = loadImgs_pytorch_src(pa_gt, ft_gt)
...
#        labs_tru = readImg_byFilesIdx(frame_idx, pa_gt, ft_gt)
    ...
    imgs = loadImgs_pytorch_src(pa_im, ft_im)
    labs = loadImgs_pytorch_src(pa_gt, ft_gt)
    fgims = loadImgs_pytorch_src(sa_pa, ft_gt)
