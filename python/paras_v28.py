...
def getSpatialData_byHistVid(vid_hist, imgs, labs, curidx, left = -1, right = 1, delta = 0.01):
    ...
    spatial_hist = spatial_hist.reshape(row_im*column_im, len_hist, 15)
...
def getListSpatialData_byHistVid(vid_hist, imgs, labs, curidx_list, mode, left = -1, right = 1, border = 0.01):

    curidx = curidx_list[0]

    hist_data, hist_labs = getSpatialData_byHistVid(vid_hist, imgs, labs, curidx, left, right, border)
    ...
    for curidx in curidx_list[1:NUM]:
        ...
        temp_data, temp_labs = getSpatialData_byHistVid(vid_hist, imgs, labs, curidx, left, right, border)
...
class ClassifyNetwork(nn.Module):
    def __init__(self, dis_num):
        ...
        self.conv1 = nn.Conv2d(15, 20, (dis_num, 1) )
        self.conv2 = nn.Conv2d(20, 500, (1, 201))
...
#     net_pa = '../../data/'
#     sa_pa = '../../data/results/'
    net_pa = '../../data/network_fountain01_v28/'
    sa_pa =  '../../data/fgimgs_fountain01_v28/'
...
#    print(torch.sum(torch.abs(vid_hist - vid_hist_pad)))
    ...
    data_vid, labs_vid = getListSpatialData_byHistVid(vid_hist, imgs, labs, curidx, "train", left_data, right_data, delta)
    print("completed")

    print(data_vid.shape)
    print(labs_vid.shape)

    print("borderline _________________________________________")
    ...
    c_W, f_W = getRandCF_multi(left, right, delta, num_dis, 15)

    c_B, f_B = getRandCF_multi(left, right, delta, num_dis, 15)
    ...
    for frame_idx in curidx:
        ...
        starttime = time.time()
        data_vid, labs_vid = getSpatialData_byHistVid(vid_hist, imgs, labs, frame_idx, left_data, right_data, delta)
        endtime = time.time()
        print("completed, total time:", endtime - starttime)
