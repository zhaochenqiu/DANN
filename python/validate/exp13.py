...
#     prodis_test = ProductDis_test.apply
    ...
    with torch.no_grad():
        ...
        X1 = torch.empty(num).normal_(mean = 3, std = 0.9)
        #+ torch.empty(num).normal_(mean = 1, std = 0.1)
        X2 = torch.empty(num).normal_(mean = 5, std = 0.7)
        ...
        c_Z, f_Z = getHist_plus(Z, border, left, right)

#     num_dis = 100
    num_dis = 1

    f_X2 = f_X2.unsqueeze(0)
    f_X1 = f_X1.unsqueeze(0)

    c_Z_W, f_Z_W = prodis_mul(c_X1, f_X1, c_X2, f_X2, c_Z, border, params)
...
#
#
#     c_W = c_W.type(torch.float32)
#     f_W = f_W.type(torch.float32)
#
#
#
#
