...
class ProductDis(Function):
    ...
    def forward(ctx, c_X, f_X, c_W, f_W, c_Z, f_Z, border = 0.1):
        ...
        with torch.no_grad():
            ...
            idx_non = ~( (pos > min_pos) & (pos < max_pos) )
...
def main():
    ...
    with torch.no_grad():
        ...
        X = torch.empty(num).normal_(mean = 10, std = 1)
        W = torch.empty(num).normal_(mean = 0, std = 4)
...
#         f_Z_ = f_Z_.to(device)
        ...
        c_Z2, f_Z2 = prodis_old(c_X, f_X, c_W, f_W, c_Z2, f_Z2, border)

        endtime = time.time()


        checkf = torch.sum( torch.abs(f_Z2 - f_Z_) )
        checkc = torch.sum( torch.abs(c_Z2 - c_Z_) )

        print("check f value:", checkf)
        print("check c value:", checkc)
#        print(f_Z2 - f_Z_)
