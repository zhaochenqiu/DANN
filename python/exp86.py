...
class ClassifyNetwork(nn.Module):
    def __init__(self, dis_num):
        ...
        self.conv1 = nn.Conv2d(3, 80, (dis_num, 1) )
        self.conv2 = nn.Conv2d(80, 2000, (1, 201))
...
#    pa_gt = '/home/cqzhao/dataset/dataset2014/dataset/dynamicBackground/fountain01/groundtruth'
    ...
    pa_fg = 'D:/projects/lab/fgimgs_fountain01_v20'
    ft_fg = 'png'

    imgs   = loadImgs_pytorch(pa_im, ft_im)
    labs   = loadImgs_pytorch(pa_gt, ft_gt)
    fgimgs = loadImgs_pytorch(pa_fg, ft_fg)
    ...
    im_vec = im.reshape(row_im*column_im, byte_im)
    lb_vec = lb.reshape(row_im*column_im)
    fg_vec = fg.reshape(row_im*column_im)
    ...
    im_vec_fg = im_vec[idx_fg, :]/255.0
    im_vec_bk = im_vec[idx_bk, :]/255.0



    print(im_vec_fg.shape)
    print(im_vec_bk.shape)



    c_R_fg, f_R_fg = getHist_plus(im_vec_fg[:, 0], 0.01, -1, 1)
    c_G_fg, f_G_fg = getHist_plus(im_vec_fg[:, 1], 0.01, -1, 1)
    c_B_fg, f_B_fg = getHist_plus(im_vec_fg[:, 2], 0.01, -1, 1)

    c_R_bk, f_R_bk = getHist_plus(im_vec_bk[:, 0], 0.01, -1, 1)
    c_G_bk, f_G_bk = getHist_plus(im_vec_bk[:, 1], 0.01, -1, 1)
    c_B_bk, f_B_bk = getHist_plus(im_vec_bk[:, 2], 0.01, -1, 1)
    ...
    plt.subplot(2, 3, 1)
    plt.bar(c_R_fg, f_R_fg,
            width = 0.01,
            color = (0.1, 0.1, 0.1, 0.5))
    plt.subplot(2, 3, 2)
    plt.bar(c_G_fg, f_G_fg,
            width = 0.01,
            color = (0.1, 0.1, 0.1, 0.5))
    plt.subplot(2, 3, 3)
    plt.bar(c_B_fg, f_B_fg,
            width = 0.01,
            color = (0.1, 0.1, 0.1, 0.5))

    plt.subplot(2, 3, 4)
    plt.bar(c_R_bk, f_R_bk,
            width = 0.01,
            color = (0.1, 0.1, 0.1, 0.5))
    plt.subplot(2, 3, 5)
    plt.bar(c_G_bk, f_G_bk,
            width = 0.01,
            color = (0.1, 0.1, 0.1, 0.5))
    plt.subplot(2, 3, 6)
    plt.bar(c_B_bk, f_B_bk,
            width = 0.01,
            color = (0.1, 0.1, 0.1, 0.5))
    ...
    plt.figure()
    plt.subplot(1, 3, 1)
    plt.imshow(im.detach().cpu().numpy()/255.0)

    plt.subplot(1, 3, 2)
    plt.imshow(lb.detach().cpu().numpy()/255.0)

    plt.subplot(1, 3, 3)
    plt.imshow(fg.detach().cpu().numpy()/255.0)
...
# #     net_pa = '../../data/'
# #     sa_pa = '../../data/results/'
#     net_pa = '../../data/network_fountain01_v21/'
#     sa_pa =  '../../data/fgimgs_fountain01_v21/'
#
#
