...
def main(argc, argv):
    ...
    name_data_vid_1 = str(argv[5])
    name_data_vid_2 = str(argv[6])
    name_data_vid_3 = str(argv[7])
    name_data_vid_4 = str(argv[8])

    name_labs_vid_1 = str(argv[9])
    name_labs_vid_2 = str(argv[10])
    name_labs_vid_3 = str(argv[11])
    name_labs_vid_4 = str(argv[12])
...
#    data_vid,    labs_vid    = getListNormalData_byHistVid_andImgs(vid_hist, trainimgs, pa_gt, ft_gt,  curidx,    "train", left_data, right_data, delta)
    ...
    data_vid_1 = torch.load(name_data_vid_1)
    data_vid_2 = torch.load(name_data_vid_2)
    data_vid_3 = torch.load(name_data_vid_3)
    data_vid_4 = torch.load(name_data_vid_4)


    labs_vid_1 = torch.load(name_labs_vid_1)
    labs_vid_2 = torch.load(name_labs_vid_2)
    labs_vid_3 = torch.load(name_labs_vid_3)
    labs_vid_4 = torch.load(name_labs_vid_4)

    data_vid = torch.cat((data_vid_1, data_vid_2, data_vid_3, data_vid_4), 0)
    labs_vid = torch.cat((labs_vid_1, labs_vid_2, labs_vid_3, labs_vid_4), 0)


    LEN_NUM = torch.numel(labs_vid)
    idx = torch.randperm(LEN_NUM)
